...
friend ostream & operator<<(ostream & out, duze & d);
friend istream & operator>>(istream & in, duze & d);
...
//********OPERATOR=**********//
    duze2 & operator=(duze & d)
    {
        this->duze::operator=(d);
        return *this;
    }

//********OPERATOR /**********//
    duze2 & operator/(duze2 & d)
    {
        int i, j;
        int pocz_rozm;
        duze2 jeden(1);
...
//********METODA POTEGA**********//
    duze2 & potega(duze2 & d)
    {
        ...
        duze2 tmp1, tmp2(2), tmp3(0), tmp4, dwa(2);
        ...
        while(d>tmp3)
        {
            ...
            {
                tmp2=tmp2*dwa;
                tmp4=tmp1*tmp1;//(a*b) mod n = ((a mod n)*(b mod n))mod n
                tmp1=tmp4;
            }
            tmp4=tmp*tmp1;
            tmp=tmp4;
...
//********METODA POTEGA MODULO**********//
    duze2 & potega_mod(duze2 & d, duze2 & n)
    {
        duze2 jeden(1);
        static duze2 tmp;
        if(tmp.wsk!=NULL)
        ...
        while(d>tmp3)
        {
            ...
            {
                tmp2=tmp2*dwa;
                tmp4=(tmp1%n)*(tmp1%n);//(a*b) mod n = ((a mod n)*(b mod n))mod n
                tmp1=tmp4%n;
            }
            tmp4=(tmp%n)*tmp1;
            tmp=tmp4%n;
...
int main()
{
    ...
    cout<<endl<<"DUZE2 3: "<<endl<<du3<<endl;
